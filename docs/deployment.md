...
- `/Users/openclaw_john/工作文件夹/acre-ui-rebuild-clean`

Active GitHub remote:

- `https://github.com/johnrebirth5-web/acre-office-warm-ui.git`
...
- sync the committed repo state into `/opt/acre-ui-rebuild/app`
- load runtime configuration from `/etc/acre/acre-ui-rebuild.env`
- restart or inspect `acre-ui-rebuild-web.service`
- validate through `http://45.55.247.137:3105/` and `http://45.55.247.137:3105/login`
- if runtime behavior disagrees with docs, trust systemd `ExecStart` and the active nginx upstream
