...
4. `/office/pipeline` 调 `@acre/db` 的 `getOfficePipelineWorkspaceSnapshot`；当前 snapshot 已拆成 `pending metrics`、`closed history metrics`、`current-month closed summary` 和 `selected rows` 几段查询，而不是先把整个可见 transaction 集一次性读回应用层；closed history 默认是最近 6 个月，也可以按 query-param 切到某个自然年的 `Jan-Dec`
