...
- that same intake assist can now also optionally use server-side OpenAI extraction when `OPENAI_API_KEY` is configured, but it still returns suggestions into the same review-first queue, keeps live-form apply manual, and falls back to the local extraction path when the API is unavailable
...
- `/agent/listings` has been retired as a standalone Front Office workspace; listing preparation now stays in `Listing Studio`, while send-risk follow-through re-enters the client dossier or calendar writeback context that owns the next action
...
- `Front Office Settings` now carries the agent's self-service public profile and avatar entry, so Listing Studio public contact cards and poster/template outputs can reuse the current account avatar while email changes still route through Admin Office requests. Avatar uploads are preview-cropped on the client and server-normalized to a 512x512 WebP image with a 5 MB original upload cap.
...
- that same `listing-studio` marketing kit now also carries `template briefs` and reusable `campaign flights`, so each poster mode has a copyable use brief and launch/event/evergreen manual cadence instead of behaving like a one-off export switcher only
...
- that same rule layer now also carries a first `review pack`, so each surfaced rule includes a copy-ready draft plus checklist guidance before the agent queues anything, still without implying auto-send or background automation
