...
- `Front Office Settings` now carries the agent's self-service public profile and avatar entry, so Listing Studio public contact cards and poster/template outputs can reuse the current account avatar while email changes still route through Admin Office requests
