...
- `/agent/calendar` now also supports a route-persistent `calendarView` contract for the main writeback lanes, so `reply due`, `confirmation pending`, `touch due`, `missing next touch`, `reschedule requested`, and `bridge logged` can reopen as stable calendar workbench states instead of ad-hoc filter combinations
...
- `/agent/dashboard` now also hardens its client and cleanup shortcuts around those focused workbench routes, so `follow first`, `anchor now`, `duplicate review`, `viewing lane`, and leadership cleanup reopen the exact next lane with explicit `why now` and `next step` copy instead of behaving like broad summary links
