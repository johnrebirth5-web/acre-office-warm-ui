...
- that same listings rail now also exposes first `profile showcase assets`, so the operator can copy an outward-facing `intro poster` and `closing history` strip from the same material center when the next send needs an agent-facing profile presentation instead of only an internal packet assembly
- that same listings rail now also exposes a first `landing-page brief` plus `case-study reel`, so the operator can prepare a fuller public-facing profile package and copy a combined showcase bundle from the same material center without pretending Acre already owns a dynamic public profile system
