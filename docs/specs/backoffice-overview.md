...
- Current notable behavior:
  ...
  - office admins can now open `Edit fields` directly inside both the `/office/transactions` create modal and `/office/transactions/new` to manage the full transaction field schema visible under `Settings > Fields > Transaction`, including direct intake fields, finance-linked fields, compatibility bridge fields, and adding a new custom field without leaving the create flow.
