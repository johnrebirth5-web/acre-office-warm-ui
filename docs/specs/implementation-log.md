...
- Current recommended FO next target after the live one-click follow-up + outbound draft assist bridge is `accepted-action history + AI outcome tracking`, so future threads can measure which grounded actions were actually accepted before introducing heavier background automation
