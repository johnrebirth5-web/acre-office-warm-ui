...
- Current recommended FO next target after live lease and appointment reminders is `deeper send records tied to appointments and client stage`, so future threads converge on the next real FO execution gap instead of jumping straight to `Offer`, `Inspection`, or AI
