...
- 2026-04-01: the first FO client dossier + BO prefill bridge is now live on top of that execution contract:
  ...
  - [apps/web/app/api/office/transactions/route.ts](/Users/openclaw_john/工作文件夹/Acre_latest_clean/apps/web/app/api/office/transactions/route.ts) now commits the handoff draft after a successful transaction create, so FO no longer leaves stale BO-ready queue items behind once the formal record exists
