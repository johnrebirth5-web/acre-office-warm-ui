...
- `/office/accounting` statement generation now supports the direct admin workflow: the primary filter action generates a payout statement immediately from eligible unpaid agent rows in the selected window, while preview remains optional for manual review/uncheck before final generation
