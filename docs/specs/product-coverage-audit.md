...
| `Activity center` | `Partial / MVP` | `/agent/notifications` now acts as a unified FO `Activity + Cleanup Center` for cleanup pressure, duplicate review, notices, shared office events, and appointment writeback pressure, including a dedicated appointment-reminder block for `confirmation due`, `reschedule follow-up`, `external touch due`, and `appointment soon`, plus basic single/bulk read-state controls and URL-persistent reminder/read-state filters, but deeper office-wide cleanup depth and external-system depth remain follow-up work. |
