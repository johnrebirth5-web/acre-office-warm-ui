...
| --- | --- | --- |
| `Dashboard` | `Live core` | Real `/agent/dashboard` with tasks, reminders, quick lead intake, send/click summary, AI queue, explainable next-touch rationale, vendor shortcuts, FO/BO split context, route-stable re-entry shortcuts into focused FO workbench lanes such as `follow first`, `anchor now`, `duplicate review`, and `viewing lane`, plus appointment commitments that now reopen exact calendar writeback focus instead of broad summary links. |
| `CRM / dossier center` | `Partial / MVP` | Real client dossier, visible-scope duplicate review / merge, and a first browser-side OCR / transcript intake assist are live; the intake review now explicitly separates recognized vs review-first vs omitted fields, duplicate warnings now steer agents into existing-record review first, `/agent/clients` now supports route-persistent `clientView` lane focus, and dossier appointment cards can reopen the exact calendar writeback flow, but deeper provider-backed intake acceleration and broader office-wide cleanup depth are still follow-up work. |
