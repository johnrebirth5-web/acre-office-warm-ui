...
| --- | --- | --- |
| `Dashboard` | `Live core` | Real `/agent/dashboard` with tasks, reminders, quick lead intake, send/click summary, AI queue, explainable next-touch rationale, vendor shortcuts, FO/BO split context, route-stable re-entry shortcuts into focused FO workbench lanes such as `follow first`, `anchor now`, `duplicate review`, and `viewing lane`, plus harder workbench CTAs for appointment writeback, send-risk follow-through, and cleanup re-entry instead of broad summary links. |
