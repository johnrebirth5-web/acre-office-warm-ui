...
| `Activity center` | `Partial / MVP` | `/agent/notifications` now acts as a unified FO `Activity + Cleanup Center` for cleanup pressure, duplicate review, notices, shared office events, and appointment writeback pressure, including next external touch deadlines on appointments, but deeper office-wide cleanup depth and external-system depth remain follow-up work. |
