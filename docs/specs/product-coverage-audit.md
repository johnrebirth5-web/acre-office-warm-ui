...
| --- | --- | --- |
| `Dashboard` | `Live core` | Real `/agent/dashboard` with tasks, reminders, quick lead intake, send/click summary, AI queue, explainable next-touch rationale, vendor shortcuts, FO/BO split context, and route-stable re-entry shortcuts into focused FO workbench lanes such as `follow first`, `anchor now`, `duplicate review`, and `viewing lane`. |
