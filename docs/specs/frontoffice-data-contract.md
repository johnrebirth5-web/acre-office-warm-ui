...
This file is the implementation-facing contract for the first real `Front Office -> Back Office` workflow bridge. It complements the product direction in [frontoffice-overview.md](frontoffice-overview.md).
...
- `/agent/calendar` now also defaults to an `Event Hub` board for `month / week / day`, while `appointmentId` and the route-stable writeback lanes still reopen the existing appointment coordination workbench
- the shared `Event` store now also powers FO office-event coordination on that same route, with `eventType`, `isOnline`, `area`, `meetingPassword`, `isMandatory`, `seriesId`, and `recurrenceRule` extending the existing event record instead of creating a parallel event system
- shared event reminders now also reconcile into the same in-app notification layer through `NotificationType.event`, so FO activity can surface near-term event pressure without inventing a background scheduler Acre does not yet have
...
- quick lead intake now also lives directly inside the active FO shell instead of forcing agents through Back Office contact admin first:
  ...
  - that same assist layer now also groups those review rows into reusable `identity`, `qualification`, `context`, `timing`, and `notes` sections, so the operator can review or apply one coherent block at a time instead of replaying the same accept flow field-by-field
  - those grouped intake sections now also carry an unresolved-first priority order plus section-level `review unresolved first` / `apply reviewed unresolved fields` actions, so the operator can clear manual-confirmation and missing-field pressure before settling the already-recognized blocks
  ...
  - the live intake surface is now intentionally slimmer than the underlying `Client` schema: AI review only writes structured `fullName`, optional `additionalFields.wechatDisplayName`, `budgetMax`, `preferredAreas`, and `followUpStatus`, while any extracted phone/email/source/stage/intent/timing context should be merged into editable `notes`
  - the visible FO display name now resolves as `wechatDisplayName || fullName`, keeping WeChat identity first without breaking the shared contact record
  - `Client` now also carries explicit FO reminder state through `followUpStatus` and `followUpReminderMode`, separating the lightweight follow-up clock from the heavier pipeline `stage`
  - automatic FO reminder cadence is now formalized as `new_lead +1 day`, `active_follow_up +2 days`, `waiting_reply +3 days`, `appointment_booked = future appointment date or +1 day`, and `paused = null`
  - manual reminder edits now lock `Client.followUpReminderMode = manual` until the agent explicitly restores `auto`, and `markFollowedUpNow` updates `Client.lastContactAt` without overriding a manually pinned reminder date
- `/agent/clients` now also exposes a pairwise duplicate review + merge surface for the current visible CRM scope:
  ...
  - that same dossier route now also resolves anchored section focus from the URL hash with shared section labels / descriptions, so cleanup, send-risk, and appointment-writeback re-entry can explain which record rail is in focus instead of silently dropping the user at the page top
  ...
  - the primary `/agent/clients` operator surface is now intentionally reduced to one queue with inline `mark followed up`, `change status`, `edit next reminder`, and `open profile` actions, while duplicate review remains a secondary block instead of the dominant page shell
  - `/agent/clients/[clientId]` is now intentionally reduced to `Client needs & basics`, `Follow-up control`, and `Note`, with older appointments / send / BO bridge / AI / lease reminder modules removed from the main page body
- shared FO reminder reconciliation now also supports client-level in-app reminders on top of the same `Client.nextFollowUpAt` clock:
  - `reconcileOfficeNotificationReminders(...)` now emits `NotificationType.follow_up` / `NotificationType.follow_up_overdue` with `NotificationEntityType.client` for due-today and overdue client reminders
  - client-level reminders are skipped whenever the same client still has any unfinished shared `FollowUpTask`, preventing a duplicated reminder signal from the lightweight client clock and the legacy task queue
  - moving `nextFollowUpAt` out of the due window or clearing it now removes the client-level reminder, keeping `/agent/notifications` aligned to the current client clock instead of stale inbox rows
- external calendar / email bridge now also lives on top of the same FO appointment foundation instead of remaining only a roadmap line:
  ...
  - that same calendar contract now also includes explicit `touch_scheduled` and `writeback_pending` aliases, so future-dated promised touches and bridge-open / writeback-missing records can reopen in stable workbench lanes instead of a mixed fallback view
  - `/agent/calendar` now also treats `month / week / day` as the shared `Event Hub` entry, so appointments and shared office events render on one board while `appointmentId` still keeps the appointment detail workbench alive
  - the FO event side of that route now reads and writes the shared `Event` model through `/api/agent/events`, `/api/agent/events/[eventId]`, `/api/agent/events/[eventId]/rsvp`, and `/api/agent/events/parse`, so RSVP, mandatory gating, recurrence presets, and review-first AI event drafts stay on the same Next + Prisma stack as the rest of FO
  ...
  - that same calendar writeback flow now also preserves the owning `calendarView` when bridge, quick-status, or preset-save actions can prove the active lane, while appointments that already have a future next touch intentionally reopen as focused records instead of a false `touch due` / `missing next touch` lane
  - `/agent/clients/[clientId]` now also reuses those lane-aware calendar hrefs and the shared listings-lane href builders across appointment cards, send records, AI accepted actions, and the next-step rail, so dossier re-entry can reopen the most relevant FO workbench instead of broad summary routes
...
- `/agent/dashboard` now also consumes the same execution-lane contracts instead of using generic summary links:
  ...
  - those appointment commitments now also choose the tightest proved `calendarView` lane before reopening the workbench, and generic listing launchers now default to `/agent/listings?lane=draft-lane` so route intent stays explicit even when no client or appointment send context has been selected yet
...
- `/agent/notifications` now also tightens cleanup re-entry instead of behaving like a broad holding page:
  ...
  - those cleanup items now also target explicit dossier section anchors such as the appointments / follow-up rail, listing-output rail, and next-step rail, so stale-client, send-risk, and appointment-writeback pressure can reopen the right part of `/agent/clients/[clientId]` instead of dropping every re-entry at the page top
  - cleanup, reminder, and notice cards now also carry a shared `sectionLabel`, so the workbench can show whether the next move belongs to calendar writeback, follow-up, listing output, recovery, appointment reminders, or general notices before the operator clicks through
...
- `/agent/listings` now also carries a route-persistent focused-lane contract on top of the existing send mode:
  ...
  - that same listings workbench now also emits a lane-execution checklist in both the main output panel and the supporting agent-material rail, so the active rescue / follow-through / draft path can show its checkpoints and next move without asking the operator to infer lane meaning from the URL alone
