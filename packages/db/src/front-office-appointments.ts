...
function strengthenFrontOfficeAppointmentCalendarExportContent(
  content: string,
) {
  const strengthenedDescription = [
    "Manual export from Acre Front Office.",
    "Acre remains the source of truth for the checkpoint.",
    "The external calendar or email draft is manual only, and no provider sync is implied by this draft.",
    "Next step: save the next checkpoint back in Acre after you open this draft or export.",
  ].join("\\n");
  ...
  return content
    ...
    .replace(
      /(\r?\nEND:VEVENT)/,
      "\r\nX-ACRE-CHECKPOINT:Manual bridge only; save the next checkpoint back in Acre.$1",
    );
}
...
function buildFrontOfficeAppointmentFollowUpPlan(input: {
    ...
    return {
      ...
      detail:
        ...
        "No next external touch set"
          ? "Acre expects another outside touch now, but the promised checkpoint was never written down."
          : `The promised checkpoint is overdue since ${input.externalWorkflow.nextActionAtLabel}.`,
      needsNextTouchPlan: false,
    };
  }

  const hasNextTouch = Boolean(input.externalWorkflow.nextActionAt);
  const needsNextTouchPlan = input.requiresExternalResponse && !hasNextTouch;
  ...
  if (
    ...
    return {
      ...
      tone: "success" as const,
      detail:
        "Outside confirmation is already saved. Add a soft reminder only if you still want a return point before the meeting.",
      needsNextTouchPlan: false,
    };
  }
...
function buildFrontOfficeAppointmentBridgeGuidance(input: {
  ...
  const manualOnlyDetail =
    "Bridge lane only: Acre logs that the bridge was opened here. The Google, Outlook, ICS, or email draft stays manual, so save the next checkpoint back on this appointment after you send or export.";
...
function buildBridgeHistoryItem(input: {
  ...
  return {
    ...
    detail:
      detailParts.join(" · ") || "Opened from the appointment record in Acre.",
...
function buildFrontOfficeAppointmentCoordinationSummary(input: {
  ...
  const nextTouchSummary = input.externalWorkflow.nextActionAt
    ...
    : input.externalWorkflow.value !==
        frontOfficeAppointmentExternalWorkflowStatuses.idle
      ? "No promised checkpoint is saved yet."
      : "";
  const noteSummary = input.externalWorkflow.note
    ? `Checkpoint note: ${input.externalWorkflow.note}.`
    : "";
    ...
    case frontOfficeAppointmentExternalWorkflowStatuses.rescheduleRequested:
      return {
        ...
        detail: [
          ...
          .join(" "),
        nextStep:
          "Use the note field to capture the new timing conversation, then save the next checkpoint until a replacement time is locked.",
    ...
    case frontOfficeAppointmentExternalWorkflowStatuses.confirmed:
      return {
        ...
        detail: [
          ...
          .join(" "),
        nextStep:
          "Leave this record confirmed, or add a soft checkpoint if you still want one before start time.",
        requiresExternalResponse: false,
        isExternalTouchDue: false,
      };
    default:
...
function appointmentMatchesSnapshotFilters(input: {
  ...
  if (normalizedFollowUp === frontOfficeAppointmentFollowUpFilters.touchDue) {
    return (
      input.appointment.statusValue === AppointmentStatus.scheduled &&
  ...
  if (
    normalizedFollowUp === frontOfficeAppointmentFollowUpFilters.touchScheduled
  ) {
    return (
      input.appointment.statusValue === AppointmentStatus.scheduled &&
      !input.appointment.isExternalTouchDue &&
...
export async function getFrontOfficeAppointmentsSnapshot(
  ...
  return {
    ...
    clientOptions: buildDedupedFrontOfficeAppointmentOptions(
      [selectedClient, ...clients].filter(isPresent).map((client) => ({
        value: client.id,
        label: `${client.fullName} · ${client.stage}`,
      })),
    ),
    listingOptions: buildDedupedFrontOfficeAppointmentOptions(
      [selectedListing, ...listings].filter(isPresent).map((listing) => ({
        value: listing.id,
...
export async function updateFrontOfficeAppointmentStatus(
  ...
  if (
    ...
    throw new Error(
      "Clear the external note and checkpoint deadline when the appointment is set back to idle.",
    );
  }
...
export async function getFrontOfficeAppointmentCalendarExport(
  ...
  return {
    ...
    content: strengthenFrontOfficeAppointmentCalendarExportContent(
      result.content,
    ),
  };
}
