...
type SignatureRequestRouteDependencies = {
  ...
  canManageOfficeSignatures?: typeof canManageOfficeSignatures;
};
...
async function runSignatureRequestAction(
  request: NextRequest,
  routeContext: Awaited<RouteContext["params"]>,
  action: SignatureRequestAction,
  context: NonNullable<
    Awaited<ReturnType<typeof getRequestSessionContext>>
  >,
  ...
  if (action === "send" || action === "resend") {
    const rateLimitDecision = await (dependencies.rateLimit ?? consumeRateLimit)(
      ...
      dependencies.rateLimitOptions ?? DEFAULT_SIGNATURE_SEND_RATE_LIMIT_OPTIONS,
    );
  ...
  try {
    ...
    if (action === "send" || action === "resend") {
      const loadSignatureEditorSnapshot =
        dependencies.getSignatureEditorSnapshot ?? getSignatureEditorSnapshot;
      ...
      const actionableRecipients = snapshot.signatureRequest.recipients.filter(
...
export async function handleSignatureRequestPatch(
  ...
  if (dependencies.withPermission) {
    ...
    return (dependencies.withPermission ?? withPermission)(
      request,
