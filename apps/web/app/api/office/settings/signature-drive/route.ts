...
function canManageSignatureDrive(currentMembership: Parameters<typeof canManageOfficeSettings>[0]) {
  return canManageOfficeSettings(currentMembership) || canManageOfficeSignatureTemplates(currentMembership);
}
...
export async function GET(request: NextRequest) {
  ...
  if (!canManageSignatureDrive(context.currentMembership)) {
    return NextResponse.json({ error: "Signature Drive settings access required." }, { status: 403 });
  }
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to save Signature Drive settings." },
      { status: 400 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to remove Signature Drive settings." },
      { status: 400 }
    );
  }
}
