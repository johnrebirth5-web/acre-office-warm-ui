...
function getReviewStatusTone(status: OfficeAgentPayoutStatementDetail["reviewStatus"]) {
  if (status === "confirmed") {
...
function buildStatementBankFields(statement: OfficeAgentPayoutStatementDetail): StatementBankField[] {
...
function StatementPostSplitCell({ lineItem }: { lineItem: OfficeAgentPayoutStatementDetail["lineItems"][number] }) {
  return (
...
export function PayoutStatementReviewClient({ statement }: PayoutStatementReviewClientProps) {
  ...
  async function handleReview(response: "confirm" | "request_revision") {
    ...
    try {
      const request = await fetch(`/api/office/accounting/self-service/statements/${statement.id}/review`, {
        method: "POST",
        headers: {
      ...
      if (!request.ok) {
        throw new Error(payload?.error ?? "Failed to submit the payout statement review.");
      }
      ...
      setError(reviewError instanceof Error ? reviewError.message : "Failed to submit the payout statement review.");
    } finally {
      setPendingAction("");
    }
  }
  ...
  return (
    <PageShell className="office-list-page office-payout-statement-page">
      <PageHeader
        actions={
          <PageHeaderSummary>
            <SummaryChip label="Status" tone="accent" value={statement.reviewStatusLabel} />
            <SummaryChip label="Period" value={statement.periodLabel} />
            <SummaryChip label="Final payout" tone="accent" value={statement.totalStatementAmountLabel} />
          </PageHeaderSummary>
        }
      ...
      <div className="office-dashboard-primary-stack">
        <SectionCard subtitle={`${statement.periodBasisLabel} · ${statement.periodLabel}`} title="Statement summary">
          <div className="office-kpi-grid office-commission-kpi-grid">
            <StatCard hint="current confirmation state" label="Review status" value={statement.reviewStatusLabel} />
            <StatCard hint="invoice-based payout subtotal" label="Invoice payout" value={statement.invoicePayoutTotalLabel} />
            <StatCard hint="manual adjustment total already applied by finance" label="Manual adjustments" value={statement.manualAdjustmentTotalLabel} />
            <StatCard hint="final statement total currently under review" label="Final payout" value={statement.totalStatementAmountLabel} />
          </div>

          <div className="office-inline-meta">
            <span>
              Generated: <LocalDateTime fallbackLabel={statement.generatedAtLabel} value={statement.generatedAt} />
            </span>
            <span>Generated by: {statement.generatedByLabel}</span>
            {statement.lastSharedAtLabel ? (
              <span>
                Last finance send: <LocalDateTime fallbackLabel={statement.lastSharedAtLabel} value={statement.lastSharedAt} />
              </span>
            ) : null}
            {statement.confirmedAtLabel ? (
              <span>
                Confirmed: <LocalDateTime fallbackLabel={statement.confirmedAtLabel} value={statement.confirmedAt} />
              </span>
            ) : null}
            <span>
              Status: <StatusBadge tone={getReviewStatusTone(statement.reviewStatus)}>{statement.reviewStatusLabel}</StatusBadge>
            </span>
            <a className="office-button-secondary office-button-sm" href={`/api/office/accounting/self-service/statements/${statement.id}/pdf`} rel="noreferrer" target="_blank">
              Download PDF
            </a>
          </div>
        </SectionCard>

        <SectionCard subtitle="The payout profile finance used for this statement snapshot." title="Payment information">
          {bankFields.length > 0 ? (
            ...
            <p className="office-form-helper">Finance has not saved payment information on your member profile yet.</p>
          )}
        </SectionCard>
        ...
        >
          ...
          <div className="office-section-actions">
            <Button disabled={!canReview || pendingAction !== ""} onClick={() => void handleReview("confirm")} type="button">
              {pendingAction === "confirm" ? "Submitting..." : "Confirm statement"}
            </Button>
            <Button
              disabled={!canReview || pendingAction !== ""}
        ...
        <SectionCard subtitle="Every finance send, reply, and agent response stays on this statement record." title="System timeline">
          {statement.timeline.length > 0 ? (
            ...
            <p className="office-form-helper">No system messages have been recorded on this statement yet.</p>
          )}
        </SectionCard>
